...
client.connect_signal("manage", function (c)
    -- Never maximized from beginning
    c.maximized = false
...
screen.connect_signal("arrange", function (s)
    ...
    local is_max = awful.layout.getname(awful.layout.get(s)) == "max"
    for _, c in pairs(s.clients) do
        if (is_max or only_one) and not c.floating or c.maximized or c.fullscreen then
            c.border_width = 0
        else
